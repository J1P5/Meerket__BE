...
public class CommentReadUsecase {
     ...
     * @return
     */
    public List<CommentReadResponseDto> getAllComments(
            Long productId, Long userId, Pageable pageable) {
        ...
        List<CommentInfo> commentInfos =
                commentService.getComments(user, productId, pageable); // 일단 부모 댓글만 가져오기
        // 엔티티 dto로 변환return CommentReadResponseDto.of().str
        return commentInfos.stream().map(CommentReadResponseDto::of).collect(Collectors.toList());
    }
}
