...
public class AreaAuthController {
    ...
    @ApiResponses(
            value = {
                @ApiResponse(responseCode = "200", description = "지역 인증 성공"),
                @ApiResponse(
                        responseCode = "400",
                        description = "1. 위도 경도 값이 범위를 벗어났습니다. \t\n" + "2. emdId는 필수 값입니다.",
                        content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
                @ApiResponse(
                        responseCode = "404",
                        description = "1. 지역인증 히스토리/유저/읍면동 지역을 찾을 수 없습니다.",
                        content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
            })
    public Response<Void> registerProfile(
            @LoginUser Long userId, @Valid @RequestBody AuthAreaRequest request) {
