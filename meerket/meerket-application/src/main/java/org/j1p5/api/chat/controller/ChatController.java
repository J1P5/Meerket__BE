...
public class ChatController {
    ...
    @ApiResponses(
            value = {
                ...
                @ApiResponse(
                        responseCode = "500",
                        description = "CHAT_ROOM_500 채팅방 업데이트 실패(안읽은 메시지 기록)")
            })
    @PostMapping("/enter/{roomId}")
    public Response<ChatRoomEnterResponse> enterChatRoom(
            @PathVariable String roomId, @LoginUser Long userId) {
    ...
    public Response<Void> exitChatRoom(@PathVariable String roomId, @LoginUser Long userId) {
    ...
    public Response<List<ChatMessageResponse>> getChatMessages(
        ...
        List<ChatMessageResponse> response =
                getChatMessageUseCase.execute(roomId, beforeTime, userId);
        return Response.onSuccess(response);
    }
    ...
    @ApiResponses(
            value = {
                ...
                @ApiResponse(
                        responseCode = "500",
                        description = "CHAT_RECEIVER_FIND_500 상대의 접속 여부를 조회중 에러 발생"),
            })
    @MessageMapping("/message")
    public Response<Void> sendChatMessage(@Payload @Validated ChatMessageRequest request) {
    ...
    public Response<List<ChatRoomInfoResponse>> getUserChatRooms(
            @LoginUser Long userId, @RequestParam ChatRoomType type) {
