...
public class FcmSenderImpl implements FcmSender {
    ...
    public void sendPushSellerBidNotification(
            Long productId, Long userId, String title, String content) {
        try {
            FcmTokenEntity fcmTokenEntity =
                    fcmTokenRepository
                            .findByUserId(userId)
                            .orElseThrow(
                                    () ->
                                            new InfraException(
                                                    FcmException
                                                            .AUCTION_SELLER_FCM_TOKEN_NOT_FOUND));

            Notification notification =
                    Notification.builder().setTitle(title + " " + content).build();
    ...
    public void sendPushBuyerBidNotification(
            Long productId, List<Long> userIds, String title, String content) {
        try {
